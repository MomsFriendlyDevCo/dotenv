	...
	*/
	types = {
		...
		'array': {
			...
			cast: v => v.split(/\s*,\s*/),
			validate: (v, field) => Array.isArray(v)
				&& (field.min === undefined || v.length >= field.min)
				&& (field.max === undefined || v.length <= field.min),
		},
		'boolean': {
		...
		'date': {
			cast: v => new Date(v),
			validate: (v, field) => v instanceof Date // Valid date object
				&& isFinite(v.getTime()) // Parses into a finite number
		...
		'emails': { // One or more emails in the
			...
			*/
			...
			validate: function(v, field) {
				...
				if (field.min !== undefined && emails.length <= field.min) throw new Error(`Minimum number of emails is ${field.min}`);
				if (field.max !== undefined && emails.length >= field.min) throw new Error(`Maximum number of emails is ${field.min}`);
				return emails
					.every(v =>
		...
		'keyvals': {
			...
			*/
			...
			validate: (v, field) =>
				typeof v == 'object'
				&& (field.min === undefined || Object.keys(v).length >= field.min)
				&& (field.max === undefined || Object.keys(v).length <= field.max),
		},
		'mongoUri': {
			cast: 'string',
		...
		'set': {
			...
			*/
			...
			validate: (v, field) =>
				v instanceof Set
				&& (field.min === undefined || v.size >= field.min)
				&& (field.max === undefined || v.size <= field.max),
		},
		'string': {
			/**
			...
			*/
			...
			validate: (v, field) =>
				...
				&& (field.min === undefined || v.length >= field.min)
				&& (field.max === undefined || v.length <= field.max),
		},
		'uri': {
			cast: 'string',
