...
* @property {Array<*>|*} [type] Either a instanceof class the object must conform to or the raw class intializer
* @property {string} [typeTitle] Human readable string describing the type - used in error reporting
* @property {*} [default] Default value to use if none is specified
* @property {function} [validator] Function to validate an input, must return true. Called as `(value)`
* @property {function} [cast] Optional function to convert user input to a native type, does not bypass validator
*/
...
export class Schema {
	...
	*/
	types = {
		...
		'boolean': {
			cast: v => ['yes', '1', 'on'].includes(v.toLowerCase()),
		},
		'date': {
			cast: v => new Date(v),
		},
	...
	*/
	applyField(val, rawFieldSchema, parent) {
		...
		if (fieldSchema.default && val === undefined)
			val = typeof fieldSchema.default == 'function' ? fieldSchema.default() : fieldSchema.default;

		// Cast
		if (fieldSchema.cast)
			val = fieldSchema.cast(val);

		// Required
		if (fieldSchema.required && val === undefined) throw new Error(`Value required`);

		// Validator|| Type checking
		if (fieldSchema.validator && !fieldSchema.validator(val))
