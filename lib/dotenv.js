...
export class DotEnv {
	...
	* @param {Boolean} [options.depth] The depth to traverse, if unspecified this defaults to false OR the last value to `deep()`
	* @param {Boolean} [options.pojoOnly=true] Do not traverse non-plain-objects
	* @returns {DotEnv} This chainable instance
	*/
	map(func, options) {
		let settings = {
		...
		let worker = (obj, currentDepth = 1) => Object.fromEntries(
			Object.entries(obj)
				.map(([key, val]) => {
					if (typeof val == 'object' && currentDepth < settings.depth - 1) { // Traverse into object?
						if (Array.isArray(val)) {
							return [
								func.call(this, [key, val])[0],
								val, // Don't mutate into arrays
							];
						} else if (!settings.pojoOnly || objUtils.isPlainObject(val)) {
							return [
								func.call(this, [key, val])[0],
	...
	* The meta variable `STATE` is also available as a key val should variable keys need to contain non-standard object notation - e.g. `${FOO-BAR}` will not result `${STATE['FOO-BAR']}` will
	...
	*/
	...
	/**
	* Run an arbitrary function passing in this DotEnv instance as the first argument and context
	* The return value of this function is then used as the new state
	* @param {Function} fn The function to run as `(dotEnv)` and `dotEnv` as the context
	* @returns {DotEnv} This chainable instance
	*/
	thru(fn) {
		this.#state = fn.call(this, this);
		return this;
	}
	// }}}
	...
	* @param {Object} [options] Additional options to mutate behaviour
	* @param {Boolean} [options.clone=false] Return a deep clone of the value - this prevents future mangling via toTree()
	* @returns {Object}
	*/
	value(options) {
		let settings = {
