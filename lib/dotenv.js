...
export class DotEnv {
	...
	*/
	schemaGlob(glob, fieldSchema) {
		...
		if (!this._schema) this._schema = new Schema();

		Object.entries(this._state)
			.filter(([key]) => matcher.test(key))
			.forEach(([key, val]) => {
				console.log('APPLY SCHEMA TO', key, 'FROM GLOB', glob, '=', fieldSchema);
				this._schema.fields[key] = fieldSchema;
				this._state[key] = this._schema.applyField(val, fieldSchema);
			})
	...
	*/
	export(options) {
		...
		if (!this._schema) throw new Error('Schema has not yet been applied');
		...
		return Object.entries(this._state)
			.map(([key, val]) => {
				...
				let fieldSchema = this._schema.getFieldSchema(this._schema.fields[key]);
	...
	*/
	toTree(options) {
		...
		this._state = Object.entries(this._state)
			.reduce((tree, [key, val]) => {
				let segments =
					settings.branches ? Array.from(settings.branches.exec(key)).slice(1)
